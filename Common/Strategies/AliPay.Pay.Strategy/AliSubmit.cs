...
namespace AliPay.Pay.Strategy
{
    ...
    public class Submit
    {
        ...
        /// 支付配置
        /// </summary>
        private static AliPayConfig PayConfig = new AliPayConfig();
        #endregion 支付配置
        ...
        static Submit()
        {
            ...
            SecurityKey = PayConfig.SecurityCheckCode;
            InputCharset = PayConfig.InputCharset;
        ...
        private static string BuildRequestMysign(Dictionary<string, string> sPara)
        {
            ...
            //把最终的字符串签名，获得签名结果
            string mysign = "";
            switch (PayConfig.SignType)
            {
                case "MD5":
                    mysign = AlipayMD5.Sign(prestr, PayConfig.SecurityCheckCode, PayConfig.InputCharset);
                    break;
                default:
                    mysign = "";
                    break;
        ...
        public static string BuildRequest(SortedDictionary<string, string> sParaTemp, string strMethod, string strButtonValue)
        {
            ...
            sbHtml.Append("<form id='alipaysubmit' name='alipaysubmit' action='" + GATEWAY_NEW + "_input_charset=" + InputCharset + "' method='" + strMethod.ToLower().Trim() + "'>");
        ...
        #endregion 建立请求，以表单HTML形式构造（默认）

        #region 建立请求，以模拟远程HTTP的POST请求方式构造并获取支付宝的处理结果
        /// <summary>
        ...
        public static string BuildRequest(SortedDictionary<string, string> sParaTemp)
        {
            ...
            //构造请求地址
            string strUrl = GATEWAY_NEW + "_input_charset=" + InputCharset;
        ...
        #endregion 建立请求，以模拟远程HTTP的POST请求方式构造并获取支付宝的处理结果

        #region 建立请求，以模拟远程HTTP的POST请求方式构造并获取支付宝的处理结果，带文件上传功能
        /// <summary>
        ...
        #endregion 建立请求，以模拟远程HTTP的POST请求方式构造并获取支付宝的处理结果，带文件上传功能

        #region 用于防钓鱼，调用接口query_timestamp来获取时间戳的处理函数
        /// <summary>
        ...
        {
            string url = GATEWAY_NEW + "service=query_timestamp&partner=" + PayConfig.PartnerID + "&_input_charset=" + PayConfig.InputCharset;
            string encrypt_key = "";
        ...
        #endregion 用于防钓鱼，调用接口query_timestamp来获取时间戳的处理函数
    }
}